...
public class CandlelightTabbedItemGroup {
    ...
    private static void initTab() {
        Tab tab = Tab.builder().predicate(
				...
				ObjectRegistry.LAMP,
				ObjectRegistry.COBBLESTONE_WOOD_FIRED_OVEN,
				ObjectRegistry.COBBLESTONE_STOVE,
				ObjectRegistry.SIDEBOARD,
				ObjectRegistry.DRAWER,
				ObjectRegistry.COBBLESTONE_KITCHEN_SINK,
        ...
        ((net.moddingplayground.frame.api.tabbeditemgroups.v0.TabbedItemGroup) Vinery.CREATIVE_TAB).getTabs().add(tab);
        tab.addToGroup(((net.moddingplayground.frame.api.tabbeditemgroups.v0.TabbedItemGroup) Vinery.CREATIVE_TAB));
    }


    public static void init() {
        Candlelight.LOGGER.debug("Initiate " + CandlelightTabbedItemGroup.class);
        initTab();
    }
}
