...
public abstract class CustomRecipeBookWidget extends DrawableHelper implements RecipeGridAligner<Ingredient>, Drawable, Element, RecipeDisplayListener {
    ...
    protected final CustomRecipeBookGhostSlots ghostSlots = new CustomRecipeBookGhostSlots();
    private final List<CustomRecipeGroupButtonWidget> tabButtons = Lists.newArrayList();
    @Nullable
    ...
    private final CustomRecipeBookRecipeArea recipesArea = new CustomRecipeBookRecipeArea();
    ...
    protected void setOpen(boolean opened) {
        ...
        this.open = opened;
        CandlelightClient.rememberedRecipeBookOpen = opened;
        if (!opened) {
    ...
    private boolean isGuiOpen() {
        return CandlelightClient.rememberedRecipeBookOpen;
    }
    ...
    private boolean toggleFilteringCraftable() {
        ...
        CandlelightClient.rememberedCraftableToggle = bl;
        return bl;
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.isOpen()) {
            ...
            for (CustomRecipeGroupButtonWidget recipeGroupButtonWidget : this.tabButtons) {
    ...
    private void drawGhostSlotTooltip(MatrixStack matrices, int x, int y, int mouseX, int mouseY) {
        ...
        for(int i = 0; i < this.ghostSlots.getSlotCount(); ++i) {
            CustomRecipeBookGhostSlots.CustomGhostInputSlot ghostInputSlot = this.ghostSlots.getSlot(i);
    ...
    public void reset() {
        ...
        this.toggleCraftableButton = new ToggleButtonWidget(i + 110, j + 12, 26, 16, CandlelightClient.rememberedCraftableToggle);
        this.setCraftableButtonTexture();
        this.tabButtons.clear();

        for (IRecipeBookGroup recipeBookGroup : screenHandler.getGroups()) {
            this.tabButtons.add(new CustomRecipeGroupButtonWidget(recipeBookGroup));
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.open && !Objects.requireNonNull(this.client.player).isSpectator()) {
            if (this.recipesArea.mouseClicked(mouseX, mouseY, button, (this.parentWidth - 147) / 2 - this.leftOffset, (this.parentHeight - 166) / 2, 147, 166)) {
                ...
                Iterator<CustomRecipeGroupButtonWidget> var6 = this.tabButtons.iterator();

                CustomRecipeGroupButtonWidget vineryRecipeGroupButtonWidget;
                do {
                    if (!var6.hasNext()) {
                        return false;
                    }
    ...
    private void refreshSearchResults() {
        ...
        this.triggerCustomEasterEgg(string);
